...
---------------------------------------------------------------------*/
bool ScriptManager_Lua::executeFile(LuaStateOwner &luaState, const string &fileName)
{
	int retVal = luaState->DoFile(fileName.c_str());
	bool success = (retVal == 0);
	if (!success) {
		LuaStackObject obj = luaState->StackTop(); // top stack object is a string containing error
...
bool ScriptManager_Lua::handleLuaFunctionEvent(const EventPtr &ePtr)
{
	...
	LuaObject luaObj = mGlobalState->GetGlobal(e.funcName().c_str());
...
{
	return events.registerEventType(eventType, RegEventPtr(new ScriptDefinedEvent()));
}
...
void ScriptManager_Lua::fireEventFromScript(const char *eventType, LuaObject &eventDataTbl, bool raise)
{
	...
	RegEventPtr rePtr = events.getRegEventPtr(eventType);
	if (!rePtr) {
		...
		if (rePtr->scriptAllowed()) {
			...
			if (raise) {
				rePtr->raiseEventFromScript(eventType, eventData);
			} else {
				rePtr->triggerEventFromScript(eventType, eventData);
			}
		} else {
...
---------------------------------------------------------------------*/
bool ScriptManager_Lua::init(const string &initFileName)
{
	// Create metatable
	mMetaTable = mGlobalState->GetGlobals().CreateTable("metaTable");
	mMetaTable.SetObject("__index", mMetaTable);

	// Make functions accessible to script
	mMetaTable.RegisterObjectDirect("doFile", (ScriptManager_Lua*)0, &ScriptManager_Lua::doFile);
	mMetaTable.RegisterObjectDirect("debugPrint", (ScriptManager_Lua*)0, &ScriptManager_Lua::debugPrint);
	mMetaTable.RegisterObjectDirect("registerHandler", (ScriptManager_Lua*)0, &ScriptManager_Lua::registerScriptHandler);
	mMetaTable.RegisterObjectDirect("unregisterHandler", (ScriptManager_Lua*)0, &ScriptManager_Lua::unregisterScriptHandler);
	mMetaTable.RegisterObjectDirect("registerEvent", (ScriptManager_Lua*)0, &ScriptManager_Lua::registerEventType);
	mMetaTable.RegisterObjectDirect("raiseEvent", (ScriptManager_Lua*)0, &ScriptManager_Lua::raiseEventFromScript);
	mMetaTable.RegisterObjectDirect("triggerEvent", (ScriptManager_Lua*)0, &ScriptManager_Lua::triggerEventFromScript);

	LuaObject luaStateManObj = mGlobalState->BoxPointer(this);
	luaStateManObj.SetMetaTable(mMetaTable);

	// Expose the metatable as a named entity.
	mGlobalState->GetGlobals().SetObject("Engine", luaStateManObj);

	// Create global actor table to hold context for all actors created in the game world
	//LuaObject globals = mGlobalState->GetGlobals();
	//LuaObject actorTable = globals.CreateTable("actorList");

	return executeFile(mGlobalState, initFileName);
}

// Constructor / destructor
ScriptManager_Lua::ScriptManager_Lua() :
	EventListener("ScriptManager_Lua"), // call the base class constructor
	mGlobalState(true)	// 'true' indicates to init the standard Lua library
{
	// register the LuaFunctionEvent with EventManager
	events.registerEventType(LuaFunctionEvent::sEventType,
...
{
	RegEventPtr rePtr = events.getRegEventPtr(ePtr->type());
